...
var logger = new winston.Logger({
    transports: [
        ...
        new winston.transports.DailyRotateFile({
            ...
            filename: 'datamanager_thumbnail_',
...
function process_controller_master() {
    ...
    async.waterfall([
        ...
        function(db_connection, waterfall_callback) {
            ...
            var sql_query = "INSERT INTO T_HEARTBEAT (create_time, type, ip) value (now(), 'DM"+acquire_code+"', '" + ipaddress[0] + "');";
            logger.debug(sql_query);
            db_connection.query(sql_query, function(err, result) {
                if(err) {
                    db_connection.release();
...
function process_controller_slave() {
    ...
    async.waterfall([
        ...
        function(db_connection, waterfall_callback) {
            // DM Hearbeat를 가져온다
            var sql_query = "SELECT *, now() as cur_time FROM T_HEARTBEAT WHERE type='DM"+acquire_code+"' ORDER BY create_time DESC limit 1; ;";
        ...
        function(db_connection, select_result, waterfall_callback) {
            ...
            if(diffsec >= manager_config.heartbeat_timeout) {
                ...
                // IP주소 가져오기
                var ipaddress = getipaddress();
                var sql_query = "INSERT INTO T_HEARTBEAT (create_time, type, ip) value (now(), 'DS"+acquire_code+"', '" + ipaddress[0] + "');";
                logger.debug("[PC_Slave] " + sql_query);
                db_connection.query(sql_query, function(err, result) {
                    if(err) {
                        db_connection.release();
                                       ...
                                       var db_clipinfo_version = 0;
                                        async.waterfall([
                                            function(each_waterfall_callback) {
                                                ...
                                                clipinfo.cpid = main_cp_info.cpid;
                                                clipinfo.corporatorcode = main_cp_info.cpid;
/*
 * thumbnail 에서도 clipid가 아닌 contentid가 key 임
 */
                                                {
 ...
 */
                                                ...
                                                if(clipinfo.uploadpath === undefined) {
                                                    logger.error("[CJ] Error : No uploadpath");
                                                    no_error = false;
                                                }
                                                if(clipinfo.uploadfilename === undefined) {
                                                    logger.error("[CJ] Error : No uploadfilename");
                                                    no_error = false;
                                                }
                                                if(no_error)
                                                {
                                                    clipinfo.originurl = path.join(clipinfo.uploadpath, clipinfo.uploadfilename);
                                                }
                                                if(clipinfo.vodpath === undefined)
                                                {
                                                    logger.error("[CJ] Error : No vodpath");
                                                    no_error = false;
                                                }
                                                clipinfo.downloadurl = clipinfo.vodpath;
                                                clipinfo.mediaurl = '';
                                                ...
                                                if(false == is_valid_number(clipinfo.interval))
                                                {
                                                    clipinfo.cliporder = 0;
                                                }else
                                                {
                                                    clipinfo.cliporder = parseInt(clipinfo.interval);
                                                }
                                                if(false == is_valid_data(clipinfo.title)) {
                                                    clipinfo.title = clipinfo.clipid;
                                                }
                                                if(!no_error) {
                                                    // Data Error
...
*/
                                                    ...
                                                    var content_path = clipinfo.originurl;
                                                    if(clipinfo.downloadurl) { content_path = clipinfo.downloadurl; }
                                                    var extdotindex = content_path.lastIndexOf('.');
                                                    var extension = content_path.slice(extdotindex+1, content_path.length).toLocaleLowerCase();
...
//*/
                                            ...
                                            function(db_connection, cpinfo, each_waterfall_callback) {
                                                if(cpinfo[0].rules === undefined || cpinfo[0].rules === null) { cpinfo[0].rules = ''; }
                                                    ...
                                                    if(err) {
                                                        ...
                                                        if(result.length <= 0) {
                                                            ...
                                                            if(db_clipinfo.mediaurl === null || db_clipinfo.mediaurl === '') {
                                                                ...
                                                                version = '1';
                                                                logger.debug("[CJ] ###2 Disable Check Version Info = " + version);
                                                                return each_waterfall_callback(null, db_connection, cpinfo, true, true);
...
//                                                                var target_path = target_path_temp.substring(0, target_path_temp.lastIndexOf('/')+1);
                                                                ...
                                                                var job_status = '0';
                                                                var thumbnail_name = path.basename(clipinfo.originurl, path.extname(clipinfo.originurl));
                                                                if(0 == downloadurl.indexOf('/')) { job_status = '20'; }
                                                                ////////////
                                                                if((main_cp_info.disable_version !== undefined && main_cp_info.disable_version === 'Y') || result.length <= 0) {
                                                                    ...
                                                                    // Query
                                                                    job_insert_query = 'INSERT INTO T_JOB (clipid, spid, cpid, rules, status, report_status, cliptype, downloadurl, target_path, version_id, itemtypeid, playtime, priority, need_report, need_copy_original, num_retry, extra_rule) VALUES (';
                                                                    ...
                                                                    {
                                                                        job_insert_query += "'"+JSON.stringify({'thumbnail': {'name': thumbnail_name,'interval': clipinfo.cliporder}})+"' ";
                                                                    }else
                                                                    {
                                                                        job_insert_query += "null";
                                                                    }
                                                                    job_insert_query += ');';
                                                                    ...
                                                                    if(status !== 70 && ((status % 10) !== 9)) {
                                                                        ...
                                                                        // Query
                                                                        job_insert_query = 'INSERT INTO T_JOB (clipid, spid, cpid, rules, status, report_status, cliptype, downloadurl, target_path, version_id, itemtypeid, playtime, priority, need_report, need_copy_original, related_job_id, num_retry, extra_rule) VALUES (';
                                                                        job_insert_query += '\'' + mysqlsinglequote(clipinfo.clipid) + '\', ';
                                                                        job_insert_query += '\'' + mysqlsinglequote(cpinfo[0].spid) + '\', ';
                                                                        job_insert_query += '\'' + mysqlsinglequote(cpinfo[0].cpid) + '\', ';
                                                                        job_insert_query += '\'' + mysqlsinglequote(cpinfo[0].rules) + '\', ';
                                           ...
                                           async.waterfall([
                                               ...
                                               function(each_db_connection, clipinfo, body, dowhile_waterfall_callback) {
                                                   ...
                                                   if((response.status === undefined || response.status === null || response.status.toUpperCase() !== 'OK')
                                                                  && (response.result === undefined || response.result === null || response.result !== true)) {
                                                       // 재시도를 하도록 한다
...
/*/
                                                                if(job.spid+'_'+job.cpid+'_'+cinfo.contentid === job.clipid && cinfo.acquire === setting_acquire) {
                                                                    find = true;
                                                                    break;
                                                                }
//*/
...
function report_result(report_url, query_string, callback)
{
    ...
    {
        callback(new Error('report_url undefined'), null);
        return;
    }
/*
 * TagStory 에서는 clipid가 아닌 contentid가 key 임
